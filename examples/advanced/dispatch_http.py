...
def create_github_server() -> MCPServer:
    ...
    async def list_repos(per_page: int = 30) -> list:
        ctx = get_context()
        response = await ctx.dispatch(HttpRequest(method=HttpMethod.GET, path=f"/user/repos?per_page={per_page}"))
        if response.success:
            return response.response.body
        return []
...
def create_multi_api_server() -> MCPServer:
    ...
    openai = Connection(
        "openai", credentials=Credentials(api_key="OPENAI_API_KEY"), base_url="https://api.openai.com/v1"
    )
    ...
    async def github_user() -> dict:
        ...
        response = await ctx.dispatch("github", HttpRequest(method=HttpMethod.GET, path="/user"))
        if response.success:
            return response.response.body
        return {"error": response.error.message}
    ...
    async def list_models() -> list:
        ...
        response = await ctx.dispatch(openai, HttpRequest(method=HttpMethod.GET, path="/models"))
        if response.success:
...
def create_robust_server() -> MCPServer:
    ...
    async def fetch_data(resource_id: str) -> dict:
        ctx = get_context()
        response = await ctx.dispatch(HttpRequest(method=HttpMethod.GET, path=f"/data/{resource_id}"))
