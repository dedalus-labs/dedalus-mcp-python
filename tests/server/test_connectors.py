...
class TestConnectorDefinition:
    ...
    def test_empty_params_raises(self) -> None:
        ...
        with pytest.raises(
            ValueError, match='params must contain at least one parameter'
        ):
            ConnectorDefinition(
                kind='test',
                params={},
    ...
    def test_empty_auth_methods_raises(self) -> None:
        ...
        with pytest.raises(
            ValueError, match='auth_methods must contain at least one method'
        ):
            ConnectorDefinition(
                kind='test',
                params={'url': str},
...
class TestConnectionTypeValidation:
    ...
    def test_validate_unsupported_auth(self) -> None:
        ...
        with pytest.raises(
            ValueError, match="auth_type 'kerberos' not in supported methods"
        ):
            conn_type.validate(handle)
...
class TestEnvironmentCredentialLoader:
    ...
    def test_missing_environment_variable_raises(
