...
class TestConnectorDefinition:
    ...
    def test_empty_kind_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="kind must be non-empty"):
            ConnectorDefinition(kind="", params={"url": str}, auth_methods=["token"])
    ...
    def test_empty_params_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="params must contain at least one parameter"):
            ConnectorDefinition(kind="test", params={}, auth_methods=["token"])
    ...
    def test_empty_auth_methods_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="auth_methods must contain at least one method"):
            ConnectorDefinition(kind="test", params={"url": str}, auth_methods=[])
    ...
    def test_to_json_multiple_types(self) -> None:
        ...
        defn = ConnectorDefinition(
            kind="custom", params={"url": str, "port": int, "timeout": float, "ssl": bool}, auth_methods=["token"]
        )
    ...
    def test_frozen_dataclass(self) -> None:
        ...
        defn = ConnectorDefinition(kind="test", params={"url": str}, auth_methods=["token"])
...
class TestConnectorHandle:
    ...
    def test_invalid_id_prefix_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="id must start with 'ddls:conn_'"):
            ConnectorHandle(id="invalid_prefix", kind="test", config={"url": "test"}, auth_type="token")

    def test_empty_kind_raises(self) -> None:
        """Test that empty kind raises ValueError."""
        with pytest.raises(ValueError, match="kind must be non-empty"):
            ConnectorHandle(id="ddls:conn_123", kind="", config={"url": "test"}, auth_type="token")
    ...
    def test_empty_config_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="config must be non-empty"):
            ConnectorHandle(id="ddls:conn_123", kind="test", config={}, auth_type="token")
    ...
    def test_empty_auth_type_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="auth_type must be non-empty"):
            ConnectorHandle(id="ddls:conn_123", kind="test", config={"url": "test"}, auth_type="")

    def test_frozen_dataclass(self) -> None:
        """Test that ConnectorHandle is immutable."""
        handle = ConnectorHandle(id="ddls:conn_123", kind="test", config={"url": "test"}, auth_type="token")
...
class TestDefineFunction:
    ...
    def test_basic_definition(self) -> None:
        ...
        conn_type = define(kind="http-api", params={"base_url": str}, auth=["service_credential"])
    ...
    def test_multiple_params_and_auth(self) -> None:
        ...
        conn_type = define(
            kind="rest_api",
            params={"base_url": str, "timeout": int, "verify_ssl": bool},
    ...
    def test_validates_on_creation(self) -> None:
        ...
        with pytest.raises(ValueError, match="kind must be non-empty"):
            define(kind="", params={"url": str}, auth=["token"])
...
class TestConnectionTypeValidation:
    ...
    def test_validate_wrong_kind(self) -> None:
        ...
        conn_type = define(kind="postgres", params={"host": str}, auth=["password"])
    ...
    def test_validate_missing_params(self) -> None:
        ...
        conn_type = define(kind="postgres", params={"host": str, "port": int, "database": str}, auth=["password"])
    ...
    def test_validate_unsupported_auth(self) -> None:
        ...
        conn_type = define(kind="postgres", params={"host": str}, auth=["password", "cert"])
    ...
    def test_validate_wrong_param_type(self) -> None:
        ...
        conn_type = define(kind="postgres", params={"host": str, "port": int}, auth=["password"])
    ...
    def test_validate_complex_scenario(self) -> None:
        ...
        conn_type = define(
            kind="rest_api", params={"base_url": str, "timeout": int, "verify_ssl": bool}, auth=["api_key", "oauth2"]
        )

        handle = ConnectorHandle(
            id="ddls:conn_xyz789",
            kind="rest_api",
            config={"base_url": "https://api.example.com", "timeout": 30, "verify_ssl": True},
            auth_type="oauth2",
        )
...
class TestIntegration:
    ...
    def test_full_workflow(self) -> None:
        ...
        handle = ConnectorHandle(
            ...
            kind="http-api",
            config={"base_url": "https://api.example.com", "label": "prod"},
            auth_type="service_credential",
        )
    ...
    def test_multiple_connection_types(self) -> None:
        ...
        RedisConn = define(kind="redis", params={"host": str, "port": int}, auth=["password", "none"])
        ...
        redis_handle = ConnectorHandle(
            id="ddls:conn_redis_1", kind="redis", config={"host": "localhost", "port": 6379}, auth_type="none"
        )
...
class TestEnvironmentCredentialLoader:
    ...
    def test_supported_auth_types(self) -> None:
        connector = define(kind="http-api", params={"base_url": str}, auth=["service_credential", "user_token"])

        loader = EnvironmentCredentialLoader(
            connector,
            variants={
                "service_credential": EnvironmentCredentials(
                    config=SecretKeys(base_url="GENERIC_BASE_URL"), secrets=SecretKeys(secret="GENERIC_SERVICE_KEY")
                ),
                "user_token": EnvironmentCredentials(
                    config=SecretKeys(base_url="GENERIC_BASE_URL"), secrets=SecretKeys(secret="GENERIC_USER_TOKEN")
                ),
            },
        )
...
class TestSecretKeysSerialization:
    ...
    def test_simple_string_bindings(self) -> None:
        ...
        assert result == {"api_key": "OPENAI_API_KEY", "org_id": "OPENAI_ORG_ID"}

    def test_binding_with_default(self) -> None:
        """Binding with default value includes it in serialization."""
        creds = SecretKeys(
            api_key="OPENAI_API_KEY", base_url=Binding("OPENAI_BASE_URL", default="https://api.openai.com/v1")
        )

        result = creds.to_dict()

        assert result["api_key"] == "OPENAI_API_KEY"
        assert result["base_url"] == {"name": "OPENAI_BASE_URL", "default": "https://api.openai.com/v1"}

    def test_binding_with_optional(self) -> None:
        """Optional binding includes flag in serialization."""
        creds = SecretKeys(api_key="OPENAI_API_KEY", org_id=Binding("OPENAI_ORG_ID", optional=True))

        result = creds.to_dict()

        assert result["api_key"] == "OPENAI_API_KEY"
        assert result["org_id"] == {"name": "OPENAI_ORG_ID", "optional": True}
    ...
    def test_binding_with_cast(self) -> None:
        ...
        assert result["timeout"] == {"name": "TIMEOUT_SECONDS", "cast": "int", "default": 30}
    ...
    def test_serialization_is_json_compatible(self) -> None:
        ...
        creds = SecretKeys(api_key="OPENAI_API_KEY", timeout=Binding("TIMEOUT", cast=int, default=30))
