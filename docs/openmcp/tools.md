...
**OpenMCP**: Decorate any callable with `@tool` inside `server.collecting()`. OpenMCP builds the input schema from type hints, wires the handler into the reference SDK, and exposes it both as a Python attribute and via `tools/call`. Allow-lists (`server.allow_tools(...)`) and `enabled` predicates give you fine-grained runtime control.
