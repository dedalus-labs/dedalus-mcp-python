...
| Prompts (`prompts/list`, `prompts/get`) | Register structured prompt templates with argument validation. | `src/openmcp/server/services/prompts.py:17-200` validates required arguments, normalises content blocks, and emits list-changed notifications. | Prompt auto-completion of argument values is not implemented server-side (aligned with spec – clients provide UX). |
...
| Sampling & elicitation proxies | Expose sampling and elicitation capability handlers required by richer clients. | `src/openmcp/server/app.py:200-334` wires `SamplingService` and `ElicitationService`. | Behaviour mirrors reference SDK; additional business logic can extend services. |
